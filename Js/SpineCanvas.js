...
(function(t) {
    var e = function() {
        ...
        t.prototype.readVarInt = function(t) {
            ...
            var e = this.readByte()
              , r = 127 & e;
            return 128 & e && (e = this.readByte(),
            r |= (127 & e) << 7,
            128 & e && (e = this.readByte(),
            r |= (127 & e) << 14,
            128 & e && (e = this.readByte(),
            r |= (127 & e) << 21,
            128 & e && (e = this.readByte(),
        ...
        t.prototype.readFloat = function() {
            return this.floatBufIn[3] = this.readByte(),
        ...
        t.prototype.readColor = function() {
            ...
            return 1 == t[0] && 1 == t[1] && 1 == t[2] && 1 == t[3] && (t = null),
            t
        }
        ,
        t
      ...
      , r = function() {
        ...
        return r.prototype.readSkeletonData = function(r) {
            ...
            for (var s = 0, p = a.readVarInt(); s < p; s++) {
                ...
                var m = a.readColor();
                null != m && (d.darkColor = new t.Color(1,1,1,1),
            ...
            for (var s = 0, y = a.readVarInt(); s < y; s++) {
                ...
                for (var x = 0, h = a.readVarInt(); x < h; x++) {
                    var v = a.readVarInt()
                      , w = i.bones[v];
                    if (null == w)
                        throw new Error("IK bone not found: " + v);
                    d.bones.push(w)
                }
                var A = a.readVarInt();
                if (d.target = i.bones[A],
                null == d.target)
            ...
            for (var s = 0, T = a.readVarInt(); s < T; s++) {
                ...
                for (var x = 0, h = a.readVarInt(); x < h; x++) {
                    var v = a.readVarInt()
                      , w = i.bones[v];
                    if (null == w)
                        throw new Error("Transform constraint bone not found: " + v);
                    d.bones.push(w)
                }
                var A = a.readVarInt();
                if (d.target = i.bones[A],
        ...
        r.prototype.readAttachment = function(e, r, i, a, o, s) {
            ...
            case t.AttachmentType.Region:
                ...
                var p = e.readColor();
                return null != p && d.color.set(p[0], p[1], p[2], p[3]),
                d.updateOffset(),
                d;
            ...
            case t.AttachmentType.Mesh:
                ...
                for (var g = [], m = 0, y = e.readVarInt(); m < y; m++)
                    g.push(e.readFloat()),
                    g.push(e.readFloat());
                for (var x = [], m = 0, w = e.readVarInt(); m < w; m++)
                    x.push(e.readShort());
                if (M.triangles = x,
                M.regionUVs = g,
            ...
            case t.AttachmentType.LinkedMesh:
                ...
                var E = e.readString()
                  , b = e.readString()
                  , C = e.readBool();
                if (M.inheritDeform = C,
                this.linkedMeshes.push(new n(M,E,i,b)),
                s) {
                    e.readFloat(),
                    e.readFloat()
                }
            ...
            case t.AttachmentType.Path:
                ...
                if (null == c)
                    return null;
                c.closed = e.readBool(),
                c.constantSpeed = e.readBool();
                var v = e.readVarInt();
                this.readVertices(e, c, v);
                for (var R = t.Utils.newArray(v / 3, 0), m = 0; m < R.length; m++)
                    R[m] = e.readFloat() * h;
                if (c.lengths = R,
                s) {
                    var p = e.readColor();
        ...
        r.prototype.readVertices = function(e, r, n) {
            ...
            r.bones = c,
            r.vertices = t.Utils.toFloatArray(u)
        }
        ,
        r.prototype.readAnimation = function(e, r, n) {
            for (var i = this.scale, a = new Array, o = 0, s = 0, h = e.readVarInt(); s < h; s++)
                for (var l = e.readVarInt(), u = 0, c = e.readVarInt(); u < c; u++) {
                    ...
                    if (d == t.TimelineType.attachment) {
                        ...
                        a.push(f),
                        o = Math.max(o, f.frames[(f.getFrameCount() - 1) * t.ColorTimeline.ENTRIES])
                    } else {
                        if (d != t.TimelineType.deform)
                            throw new Error("Invalid timeline type for a slot: " + d + " (" + l + ")");
                        ...
                        a.push(f),
                        o = Math.max(o, f.frames[(f.getFrameCount() - 1) * t.TwoColorTimeline.ENTRIES])
                    }
                }
                ...
                for (var E = e.readVarInt(), u = 0, c = e.readVarInt(); u < c; u++) {
                    ...
                    if (d === t.TimelineType.rotate) {
                        ...
                        for (var v = 0, b = 0; b < p; b++)
                            ...
                            v++;
                        a.push(f),
                        o = Math.max(o, f.frames[(f.getFrameCount() - 1) * t.RotateTimeline.ENTRIES])
                    } else {
                        ...
                        var f = null
                          , C = 1;
                        d === t.TimelineType.scale ? f = new t.ScaleTimeline(p) : d === t.TimelineType.shear ? f = new t.ShearTimeline(p) : (f = new t.TranslateTimeline(p),
                        C = i),
                        f.boneIndex = E;
                        ...
                        a.push(f),
                        o = Math.max(o, f.frames[(f.getFrameCount() - 1) * t.TranslateTimeline.ENTRIES])
                    }
                }
            for (var s = 0, P = e.readVarInt(); s < P; s++) {
                ...
                for (var v = 0, F = 0; F < p; F++)
                    ...
                    v++;
                a.push(f),
                o = Math.max(o, f.frames[(f.getFrameCount() - 1) * t.IkConstraintTimeline.ENTRIES])
            }
            for (var s = 0, P = e.readVarInt(); s < P; s++) {
                var L = e.readVarInt()
                  , p = e.readVarInt()
                ...
                for (var v = 0, _ = 0; _ < p; _++)
                    f.setFrame(v, e.readFloat(), e.readFloat(), e.readFloat(), e.readFloat(), e.readFloat()),
                    v < p - 1 && this.readCurve(e, f, v),
                    v++;
                a.push(f),
                o = Math.max(o, f.frames[(f.getFrameCount() - 1) * t.TransformConstraintTimeline.ENTRIES])
            }
            for (var s = 0, P = e.readVarInt(); s < P; s++)
                for (var k = e.readVarInt(), N = n.pathConstraints[k], u = 0, D = e.readVarInt(); u < D; u++) {
                    ...
                    if (d === t.TimelineType.pathConstraintPosition || d === t.TimelineType.pathConstraintSpacing) {
                        var f = null
                          , C = 1;
                        d === t.TimelineType.pathConstraintSpacing ? (f = new t.PathConstraintSpacingTimeline(p),
                        N.spacingMode != t.SpacingMode.Length && N.spacingMode != t.SpacingMode.Fixed || (C = i)) : (f = new t.PathConstraintPositionTimeline(p),
                        N.positionMode == t.PositionMode.Fixed && (C = i)),
                        ...
                        for (var v = 0, B = 0; B < p; B++)
                            ...
                            v++;
                        a.push(f),
                        o = Math.max(o, f.frames[(f.getFrameCount() - 1) * t.PathConstraintPositionTimeline.ENTRIES])
                    } else if (d === t.TimelineType.pathConstraintMix) {
                        var f = new t.PathConstraintMixTimeline(p);
                        f.pathConstraintIndex = k;
                        for (var v = 0, U = 0; U < p; U++)
                            f.setFrame(v, e.readFloat(), e.readFloat(), e.readFloat()),
                            v < p - 1 && this.readCurve(e, f, v),
                            v++;
                        a.push(f),
                        o = Math.max(o, f.frames[(f.getFrameCount() - 1) * t.PathConstraintMixTimeline.ENTRIES])
                    }
                }
            for (var s = 0, P = e.readVarInt(); s < P; s++) {
                var O = e.readVarInt()
                  , X = n.skins[O];
                if (null == X)
                    throw new Error("Skin not found: " + O);
